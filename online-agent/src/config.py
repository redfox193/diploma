...
class DisturbanceConfig:
    """Описание возмущения (задержки) для online-сценария."""
    kind: str
...
def load_environment_config(config_path: str) -> EnvironmentConfig:
    ...
    return EnvironmentConfig(
        ...
        time_window_minutes=float(raw.get("time_window_minutes", 60.0)),
    )
...
@dataclass
class OnlineScenarioConfig:
    """Конфигурация online-сценария с возмущениями."""
    disturbances: List[DisturbanceConfig]
    offline_checkpoint_path: str


def load_online_scenario_config(config_path: str) -> OnlineScenarioConfig:
    with Path(config_path).open("r", encoding="utf-8") as f:
        raw = yaml.safe_load(f)
    ...
    disturbances = [
        ...
        for d in raw.get("disturbances", [])
    ]

    return OnlineScenarioConfig(
        disturbances=disturbances,
        offline_checkpoint_path=raw["offline_checkpoint_path"],
    )

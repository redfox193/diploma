...
from .config import DisturbanceConfig, EnvironmentConfig, LineData
...
class _BusState:
    ...
    def clone(self) -> _BusState:
        """Создать копию состояния водителя."""
        return _BusState(
            bus_id=self.bus_id,
            home_cp=self.home_cp,
            current_cp=self.current_cp,
            shift_duration=self.shift_duration,
            work_duration_before_lunch=self.work_duration_before_lunch,
...
class Environment:
    """Симулятор для offline/online обучения RL-агента задачи CSP."""

    _FEATURES_PER_CP = 5
    _FEATURES_PER_LINE = 3
    _FEATURES_PER_CANDIDATE = 6

    def __init__(self, config: EnvironmentConfig, mode: str = "offline") -> None:
        self._config = config
        self._mode = mode
        self._disturbances = config.disturbances
        ...
        self._deadhead_policy = None
        self._deadhead_planner: Optional[TimeWindowDeadheadPlanner] = None
    ...
    def next_departure_time(self) -> float:
        """Время следующего рейса (используется планировщиком)."""
        if self._current_step + 1 < self._total_departures:
            return self._departure_sequence[self._current_step + 1].departure_time
        return self._current_time

    def attach_deadhead_policy(self, policy) -> None:
        """Привязать обученную offline-политику для планировщика холостых поездок."""
        self._deadhead_policy = policy
    ...
    def _build_action_candidates(self) -> List[Optional[_CandidateBus]]:
        ...
        if self._mode == "online":
            ordered_groups = [used_no_dh, unused_no_dh]
        else:
    ...
    def _apply_action(self, candidate: _CandidateBus) -> float:
        ...
        actual_travel_time = self._travel_time(dep)
        arrival_time = dep.departure_time + actual_travel_time
    ...
    def _travel_time(self, departure: _DepartureEvent) -> float:
        """Время движения по линии с учётом возмущений (online-режим)."""
        travel_time = departure.travel_time
        if self._mode == "online":
            for d in self._disturbances:
                if d.line_id and d.line_id != departure.line_id:
                    continue
                if d.start_minute is not None and departure.departure_time < d.start_minute:
                    continue
                if d.end_minute is not None and departure.departure_time > d.end_minute:
                    continue
                travel_time += d.delay_minutes
        return travel_time

    def _configure_deadhead_planner(self) -> None:
        """Создать TimeWindowDeadheadPlanner при наличии offline-политики."""
        if self._mode == "online" and self._deadhead_policy is not None:
            self._deadhead_planner = TimeWindowDeadheadPlanner(
                time_window=self._config.time_window_minutes,
                min_rest=self._min_rest,
                policy=self._deadhead_policy,
            )
        else:
            self._deadhead_planner = None

    def _maybe_dispatch_deadheads(self) -> None:
        """Вызвать планировщик холостых поездок после каждого действия (online-режим)."""
        if self._deadhead_planner is None or self._current_departure is None:
            return
        self._deadhead_planner.plan(self)

    def dispatch_deadhead(
        self, bus_id: str, target_cp: str, deadhead_time: float
    ) -> bool:
        """Отправить автобус в холостую поездку (вызывается планировщиком)."""
        ...
        bus.available_time = self._current_time + deadhead_time
        source_cp = bus.current_cp
        bus.current_cp = None
        self._total_deadhead_time += deadhead_time

        self._history.append({
            "event": "deadhead",
            "bus_id": bus.bus_id,
            "cp_id_from": source_cp,
            "cp_id_to": target_cp,
            "start_time": self._current_time,
            "end_time": self._current_time + deadhead_time,
        })
        return True

    def clone_for_planning(self) -> Environment:
        """Лёгкая копия окружения для планировщика (без рекурсии)."""
        clone = Environment.__new__(Environment)
        clone._config = self._config
        clone._lines = self._lines
        clone._cp_ids = self._cp_ids
        clone._cp_index = self._cp_index
        clone._num_cps = self._num_cps
        clone._departure_sequence = self._departure_sequence
        clone._total_departures = self._total_departures
        clone._cp_departure_times = self._cp_departure_times
        clone._target_action_size = self._target_action_size
        clone._short_horizon = self._short_horizon
        clone._min_rest = self._min_rest
        clone._time_norm = self._time_norm
        clone._fleet_distribution = self._fleet_distribution
        clone._mode = "offline"
        clone._disturbances = self._disturbances
        clone._buses = {bid: bus.clone() for bid, bus in self._buses.items()}
        clone._current_step = self._current_step
        clone._current_departure = self._current_departure
        clone._current_time = self._current_time
        clone._current_candidates = []
        clone._current_action_mask = np.zeros(self._target_action_size, dtype=np.float32)
        clone._total_deadhead_time = self._total_deadhead_time
        clone._accumulated_reward = self._accumulated_reward
        clone._final_reward_value = self._final_reward_value
        clone._invalid_actions = self._invalid_actions
        clone._uncovered_departures = self._uncovered_departures
        clone._history = list(self._history)
        clone._next_auto_bus_idx = self._next_auto_bus_idx
        clone._deadhead_policy = None
        clone._deadhead_planner = None
        return clone


class TimeWindowDeadheadPlanner:
    """Механизм временного окна: использует offline-политику для упреждающей
    отправки автобусов в холостые поездки."""
    ...
    def plan(self, env: Environment) -> None:
        """Просмотреть будущие рейсы и диспетчеризовать холостые поездки."""
        if self.policy is None or env._current_departure is None:
            return

        window_end = env._current_time + self.time_window
        planner_env = env.clone_for_planning()
        obs = planner_env._build_observation()

        while (
            planner_env._current_departure is not None
            and planner_env._current_departure.departure_time <= window_end
        ):
            mask = planner_env.action_mask
            action, _, _ = self.policy.act(obs, mask, deterministic=True)
            candidate = planner_env._current_candidates[action]

            if candidate and candidate.requires_deadhead:
                dep = planner_env._current_departure
                latest_depart = (
                    dep.departure_time - self.min_rest - candidate.deadhead_time
                )
                if env._current_time <= latest_depart <= env.next_departure_time:
